...
#define _lrc_n_arr_k(...)                                                      \
  (sizeof((uint8_t[]){__VA_ARGS__}) / sizeof(uint8_t)), (uint8_t[]) {          \
    __VA_ARGS__                                                                \
  }
 ...
 * ->   lrc_init_n(lrc, (sizeof((uint8_t[]){2, 3}) / sizeof(uint8_t)),
 * (uint8_t[]){2, 3}, 2)
...
extern int jerasure_matrix_decode(int k, int m, int w, int *matrix,
...
typedef struct {
  ...
  int *decode_matrix; /* matrix with damaged data-row/unnecessary code-row
                         removed */
